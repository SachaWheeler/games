...
def fill_grid(grid, row, N, wordlist):
    ...
    for word in x_words:
        ...
        if REUSE_WORDS and [word[i] for i in range(row)] != [
            grid[j][row] for j in range(row)
        ]:
            continue
        ...
        # debugging
        print(
            ["".join(sublist) for sublist in grid],
            f"{NUMBER_RUNS:,}",
            "               ",
            end="\r",
        )
        ...
        if all(
            has_valid_column_match(grid, col, N, y_words, used_words_in_grid(grid))
            for col in range(X)
        ):
    ...
    try:
        with open(FILENAME, "r") as file:
            wordlist = set(
                word.strip().lower()
                for word in file
                if len(word.strip()) in N and word.strip().isalpha()
            )
    except:
        wordlist = set(
            word.strip().lower()
